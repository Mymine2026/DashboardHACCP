...
async function load(){
  ...
  try{
    if(!devId){
      ...
      devId=dev.id;\n      // Show frigo name\n      const dFrigo=document.getElementById(\'dFrigo\');\n      if(dFrigo)dFrigo.textContent=nomeFrigo;\n      // Build frigo tabs if multiple sensors\n      const tabsEl=document.getElementById(\'frigoTabs\');\n      if(tabsEl&&cd?.sensori&&cd.sensori.length>1){\n        tabsEl.style.display=\'flex\';\n        tabsEl.innerHTML=cd.sensori.map((s,i)=>\n          \'<a href="?client=\'+ci+\'&sensore=\'+i+\'"\'+(i===_si?\' style="background:var(--green);color:#fff;border-color:var(--green)"\':\'\')+\' class="btn" style="font-size:11px">&#10052; \'+( s.nome_frigo||s.eui.slice(-6))+ \'</a>\'\n        ).join(\'\');\n      }
