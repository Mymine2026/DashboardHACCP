...
def _build_pdf(nome, client, date_str, hour_rows, all_T, all_H, raw_rows):
    ...
    import base64 as _b64i, struct as _struct, zlib as _zlib
    _png_bytes = _b64i.b64decode(HACCP_LOGO_B64)
    # Robust PNG→raw RGB using built-in struct+zlib only (no PIL needed at runtime)
    def _png_to_raw_rgb(data):
        import struct, zlib
        if data[:8] != b"\x89PNG\r\n\x1a\n":
            raise ValueError("Not a PNG")
        pos, idat, w, h, ct, bd = 8, b"", 0, 0, 2, 8
        while pos < len(data) - 4:
            ...
            if ct_name == b"IHDR":
                ...
                bd, ct = chunk[8], chunk[9]  # bit depth, color type
            elif ct_name == b"IDAT":
                idat += chunk
            elif ct_name == b"IEND":
                break
        ...
        stride = w * ch + 1
        # Reconstruct pixels applying PNG filters
        px_data = [bytearray(stride-1) for _ in range(h)]
        for r in range(h):
            filt = raw[r*stride]
            row = bytearray(raw[r*stride+1 : r*stride+stride])
            prev = px_data[r-1] if r > 0 else bytearray(stride-1)
            if filt == 1:   # Sub
            ...
            px_data[r] = row
        # Extract RGB (drop alpha if present)
        rgb = bytearray()
        for row in px_data:
            for p in range(w):
                base = p * ch
                if ct in (2, 3):   # RGB or Indexed→treat as RGB
                    rgb += bytes(row[base:base+3])
                elif ct == 6:      # RGBA: composite on white
                    r2,g,b2,a = row[base],row[base+1],row[base+2],row[base+3]
                    a_f = a/255.0
                    rgb += bytes([int(r2*a_f+255*(1-a_f)),
                                  int(g*a_f+255*(1-a_f)),
                                  int(b2*a_f+255*(1-a_f))])
                else:              # Grayscale → expand to RGB
                    v = row[base]; rgb += bytes([v,v,v])
        return w, h, bytes(rgb)
    try:
        _iw, _ih, _rgb = _png_to_raw_rgb(_png_bytes)
        _img_stream = _zlib.compress(_rgb, 6)
        ...
        print(f"  [PDF] logo embed error: {_e}")
...
HACCP_LOGO_B64 = ""
