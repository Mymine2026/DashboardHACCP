...
def generate_pdf_report(client, tipo="giornaliero", anno=None, mese=None, data=None):
    """
    Genera PDF HACCP.
    tipo='giornaliero': misurazioni di un giorno specifico ogni 4 ore
      - data='YYYY-MM-DD' per giorno specifico, altrimenti ieri
    tipo='mensile': 1 riga/giorno con T.Min / T.Max / T.Media per tutto il mese
    """
    try:
        ...
        for _si_idx, _s in enumerate(client.get("sensori", [{"eui": client.get("eui","")}])):
            ...
            if tipo == "giornaliero":
                if data:
                    try: yday = datetime.strptime(data, "%Y-%m-%d").date()
                    except: yday = (datetime.now() - timedelta(days=1)).date()
                else:
                ...
                if anno and mese:
                    ...
                    last_month_end = datetime(int(anno), int(mese),
                                             calendar.monthrange(int(anno), int(mese))[1]).date()
                else:
                    first_today = datetime.now().date().replace(day=1)
                    last_month_end = first_today - timedelta(days=1)
                    last_month_start = last_month_end.replace(day=1)
                rows_4h = []  # una entry per giorno
                d = last_month_start
                while d <= last_month_end:
                    day_frames = _fetch_frames(dev_id, d)
                    temps = [r["T"] for r in day_frames if r.get("T") is not None]
                    hums  = [r["H"] for r in day_frames if r.get("H") is not None]
                    rows_4h.append({
                        ...
                        "H_avg":   round(sum(hums)/len(hums), 0) if hums else None,
                    })
                    d += timedelta(days=1)
...
def _build_pdf(nome, client, date_str, rows_4h, mese_anno, tipo="giornaliero"):
    ...
    # cols dipendono dal tipo
    if tipo == "mensile":
        # Giorno | Sensore | T.Min | T.Max | T.Media | Umid.Med | Note/Azioni  — sum=525
        COLS  = [60, 100, 52, 52, 55, 50, 156]
        CLBLS = ["Giorno","Sensore","T.Min(C)","T.Max(C)","T.Media","Umid.(%)","Note / Azioni Correttive"]
    else:
        # Giorno|Ora|Sensore|Temp|Umid|Note|Azioni  — sum=525
    ...
    # ── TABLE ROWS ──
    # Recupera soglie cliente per evidenziare allarmi
    _sens0 = (client.get("sensori") or [{}])[0]
    _t_min_cfg = _sens0.get("t_min") if _sens0.get("t_min") is not None else client.get("t_min")
    _t_max_cfg = _sens0.get("t_max") if _sens0.get("t_max") is not None else client.get("t_max")
    ...
    for ri, row in enumerate(rows_4h):
        ...
        if tipo == "mensile":
            T_avg_val = row.get("T_avg")
            T_min_val = row.get("T_min")
            T_max_val = row.get("T_max")
            H_avg_val = row.get("H_avg")
            alarm = (T_avg_val is not None and (
                (_t_min_cfg is not None and T_avg_val < float(_t_min_cfg)) or
                (_t_max_cfg is not None and T_avg_val > float(_t_max_cfg))
            ))
            ...
            vals = [
                row.get("giorno",""),
                row.get("sensore","")[:18],
                fmt(T_min_val) if T_min_val is not None else u"—",
                fmt(T_max_val) if T_max_val is not None else u"—",
                fmt(T_avg_val) if T_avg_val is not None else u"—",
                fmt(H_avg_val, 0) if H_avg_val is not None else u"—",
                "",
            ]
            cx = LM
            for i,(v,cw) in enumerate(zip(vals,COLS)):
                col = RED if (alarm and 2 <= i <= 4) else BLACK
                if i <= 1: txt(cx+3, y-12, "F1", 8, col, v)
            ...
            alarm = (T_val is not None and (
                (_t_min_cfg is not None and T_val < float(_t_min_cfg)) or
                (_t_max_cfg is not None and T_val > float(_t_max_cfg))
            ))
            if alarm: bg = "1.0 0.88 0.88"
            filledbox(LM, y-RH, TW, RH, bg)
            hline(y-RH, lw=0.2, rgb=LGREY)
...
function dlR(e){e.preventDefault();window.location.href='/day-select?client='+ci;}
function dlM(e){e.preventDefault();window.location.href='/reports?client='+ci;}
...
<div class="card">
  ...
  <div class="note">Il PDF include T.Min, T.Max e T.Media per ogni giorno del mese selezionato</div>
            ...
            _data=qs.get("data",[None])[0]
            pdf,err=generate_pdf_report(client, tipo=tipo, anno=_anno, mese=_mese, data=_data)
            ...
            if tipo=="mensile":
                ...
                if _data:
                    try: dt_str=datetime.strptime(_data,"%Y-%m-%d").strftime("%Y%m%d")
                    except: dt_str=(datetime.now()-timedelta(days=1)).strftime("%Y%m%d")
                else:
