...
API_KEY = "YOUR_TRACKPAC_API_KEY"
BASE    = "https://v2-api.trackpac.io"
PORT    = 8765
BUILD_TS    = '2026-03-14 16:19:26'
DATA        = os.path.join(os.path.dirname(os.path.abspath(__file__)), "clients.json")
...
SMTP_PORT = 587
SMTP_USER = ""   # es: alerts@mymine.io
SMTP_PASS = ""   # App Password Gmail 16 caratteri senza spazi
SMTP_FROM = "MyMine Alerts <" + SMTP_USER + ">"
...
TWILIO_ACCOUNT_SID = ""   # es: "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"   # es: "ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
TWILIO_AUTH_TOKEN  = ""   # Auth Token dalla console Twilio   # Auth Token dalla console Twilio
TWILIO_FROM_NUMBER = ""   # es: "+15551234567"   # es: "+15551234567"
...
def _build_pdf(nome, client, date_str, hour_rows, all_T, all_H, raw_rows):
    ...
    txt(ML+12, H-50, "F2", 20, "1 1 1", "mymine")
    txt(ML+12, H-64, "F1", 8,  "0.5 0.8 0.65", "REPORT MISURAZIONI AMBIENTALI")
...
.navcap{font-family:var(--mono);font-size:10px;color:var(--dim);letter-spacing:.12em;text-transform:uppercase}
...
.panel-sub{font-family:var(--mono);font-size:9px;color:var(--dim);letter-spacing:.12em;text-transform:uppercase;margin-bottom:20px}
.row2{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.field{margin-bottom:14px}
.field label{display:block;font-family:var(--mono);font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:var(--sub);margin-bottom:5px}
...
.ccard-name{font-size:14px;font-weight:700;margin-bottom:5px;padding-right:70px}
.ccard-details{font-family:var(--mono);font-size:10px;color:var(--sub);line-height:1.9}
.ccard-eui{color:var(--green);font-weight:500}
.btn-creds{position:absolute;bottom:10px;right:12px;background:none;border:1px solid rgba(29,181,132,.2);color:var(--sub);border-radius:6px;padding:2px 8px;font-size:9px;cursor:pointer;font-family:var(--mono);transition:all .2s}
.btn-creds:hover{border-color:var(--green);color:var(--green2)}
.btn-edit{position:absolute;top:12px;right:76px;background:none;border:1px solid rgba(29,181,132,.25);color:var(--green2);border-radius:6px;padding:3px 9px;font-size:10px;cursor:pointer;font-family:var(--mono);transition:all .2s}
.btn-edit:hover{background:rgba(29,181,132,.08);border-color:var(--green)}
.btn-del{position:absolute;top:12px;right:12px;background:none;border:1px solid rgba(217,79,79,.25);color:var(--red);border-radius:6px;padding:3px 9px;font-size:10px;cursor:pointer;font-family:var(--mono);transition:all .2s}
...
.sec{font-family:var(--mono);font-size:9px;letter-spacing:.14em;text-transform:uppercase;color:var(--green2);font-weight:600;margin:14px 0 10px;display:flex;align-items:center;gap:8px}
...
.btn-check{width:100%;background:var(--bg3);border:1px solid var(--line);color:var(--green2);border-radius:9px;padding:9px;font-family:var(--sans);font-size:12px;font-weight:600;cursor:pointer;transition:all .2s;margin-bottom:14px}
  ...
  try{
    ...
    box.innerHTML=cls.map((c,i)=>{
      ...
      return`<div class="ccard${hasAlarm?' alarm':''}" onclick="go(${i})">
        <button class="btn-del" onclick="event.stopPropagation();del(${i})">✕ Elimina</button>
      <button class="btn-edit" onclick="event.stopPropagation();editClient(${i})">✎ Modifica</button>
      <button class="btn-creds" onclick="event.stopPropagation();sendCreds(${i})" title="Invia credenziali">✉ Credenziali</button>
...
def daily_report_thread():
    """Invia report PDF giornaliero alle 9:00 per ogni cliente con email."""
    import time as _t
    while True:
        now = datetime.now()
        # Calcola secondi fino alle 9:00 di domani (o oggi se non ancora passato)
        target = now.replace(hour=9, minute=0, second=0, microsecond=0)
        if now >= target:
            target += timedelta(days=1)
        wait = (target - now).total_seconds()
        print(f"  [REPORT] Prossimo invio: {target.strftime('%Y-%m-%d 09:00')}")
        _t.sleep(wait)
        try:
            send_daily_reports()
        except Exception as e:
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/","/index.html"): self.send_html(HTML_CLIENTS_FINAL)
        elif path=="/dashboard": self.send_html(HTML_DASH_FINAL)
        elif path=="/proxy":
